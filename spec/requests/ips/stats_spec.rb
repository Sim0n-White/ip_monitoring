...
RSpec.describe "GET /ips/:id/stats", type: [:request, :db] do
  let(:ip_repo) { IpMonitoring::Repos::IpRepo.new }
  let(:ip_check_repo) { IpMonitoring::Repos::IpCheckRepo.new }
  ...
  describe "valid requests" do
    context "when data is available" do
      let!(:checks) do
        checks = Array.new(3) { { ip_id: ip.id, rtt: rand(0.01..1.00), checked_at: Time.now } } +
          Array.new(2) { { ip_id: ip.id, rtt: nil, checked_at: Time.now } }
  ...
  describe "invalid requests" do
    context "when parameters are missing" do
      it "returns 422 with errors" do
        get "/ips/#{ip.id}/stats", params.delete(:time_from)
    ...
    context "when data is unavailable" do
      ...
      it "returns 404 with error message" do
        get "/ips/#{ip.id}/stats", no_data_params
